...
# V1.2 - Python 2 and Python 3 compatible
# V1.3 - changed to http 

version = "AMES sendfile.py V1.3"
...
def submit(content,ext,pin):
    ...
    #print("submit ping=",ping())#debug
    if ping():
        try:
            print("submit ping= True")#debug
            #ping()
            try:
                #Python 3
                req = urllib.request.Request('http://ames.herts.ac.uk/cgi-bin/storefile.py',body,headers)
                response = urllib.request.urlopen(req).read().decode('utf-8')
            except AttributeError:
                # Python 2
                #raise# debug
                req = urllib2.Request('http://ames.herts.ac.uk/cgi-bin/storefile.py',body,headers)
...
def download(pin,ext):
    ...
    """
    url = 'http://ames.herts.ac.uk/cgi-bin/fetchfile.py'
    timeleft=0 # spaceholder for Alex's timeleft variable
    #print("download ping=",ping())#debug
    if ping():
        try:
            print("download ping=True")
            #ping()
            try:
                #Python 3
...
def ping(host="ames.herts.ac.uk"):
    ...
    if  platform.system().lower()=="windows":
        ...
        return subprocess.call("ping -n 1 -t 1000" + host, creationflags=DETACHED_PROCESS) #Windows version!
    elif platform.system().lower()=="darwin":
        # OSX/MAC
        return os.system("ping -c 1 -W 1000 " + host+"> /dev/null") == 0 # OSX, timeout in ms
    else:
        # Linux
        return os.system("ping -c 1 -W 1 " + host+"> /dev/null") == 0 # timeout in secs, Works ok on windows, but open DOS win!
