...
class CocoIDE(tk.Frame):
    ...
    def __init__(self, Emulator=None, master=None, filename=None, name='cocoide'):
        ...
        self.editmenu.add_command(label="Cut", command=self.cut, accelerator=comkey+"x")
        self.editmenu.add_command(label="Copy", command=self.copy, accelerator=comkey+"c")
        self.editmenu.add_command(label="Paste", command=self.paste, accelerator=comkey+"v")
        self.editmenu.add_separator()
        self.txtmenu = tk.Menu(self.editmenu, tearoff=0)
        self.txtmenu.add_radiobutton(label="V Large", command=lambda: self.changeTextSize(16))
        ...
        self.runStopButton = tk.Button(buttonBar, text="Start ", height=3, padx=6,
            width=4, command=self.runProg)
        ...
        # Cludge for Mac/OSX
        if platform == "darwin":
            compileButton = tk.Button(buttonBar, text="Compile/Reset ", command=self.compileText, height=3, padx=6)
        else:
        ...
        self.lntext = tk.Text(mainPanel, width = 4,
                ...
                takefocus = 0, bd = 2, background ='lightgrey',
        ...
        watchHeader = tk.Label(watchPanel, text="Pg:  Adr:  Label:               Content ")
        ...
        for n in range(8):
            self.pageRadButtons.append(tk.Radiobutton(self.memPageFrame, text=str(n),
                ...
                command=lambda: (self.dispAllMemory(page=self.memPageVar.get()), self.dispPC())))
        ...
        if  self.Emu.memChanged[0]:
            ...
            for port in self.IOPorts:
                ...
                if adr in port.getOPadr():
                    ...
                    errormsg = port.setOPval(adr, self.Emu.memory[0][self.Emu.datamem[0]][adr])
                    if errormsg:
                        #rint(errormsg)
                        txtaddr = self.mcode_list.search(self.Emu.hx(self.Emu.PC-1)+":", "1.0", tk.END)
                        if txtaddr:
                            #print("£",txtaddr)
                            self.mcode_list.tag_add("smod", "%s linestart" % txtaddr, "%s lineend+1c" % txtaddr) # add tag to k
    ...
    def amesStart(self, event=None):
        ...
        if errormsg == None: # No network errors etc.
            ...
            if success:# and self.timeleft>0: #if errormsg == None: # commnent out for testing w out ames.
    ...
    def amesStop(self):
        ...
        if self.amesSubmit() == True: # if file submitted ok
            ...
            self.statusMsg.config(text="Problem submitting file:\nPlease Save file as yourSRN.asm\n and Email to Tutor")
            self.file_save_as()
            self.file_new()
        ...
        for n in range(len(self.Emu.memory[self.Emu.curPage])):
            for gridy in range(1,17):
                for gridx in range(1, 17):
                    #print(n, "*", index, end=":")
                    self.memLabel[index] = tk.Label(self.mem_frame[n], font=self.defaulttxtfont, text="00", width=2,fg="grey", bg="white")#, font=self.smallfont)
    ...
    def dispAllMemory(self, page=None):#?? 
        ...
        for n in range(numbanks): # All banks in in current CDM8 memory page
            ...
            for memval in self.Emu.memory[curPage][n]:#self.Emu.curPage][n]: # For each bank
                self.memLabel[index].config(text=self.Emu.hx(memval),fg=cf.memColour, bg=cf.membgColour)#memColour
                ...
                if self.memChanged:
                    ...
                    if  self.arch[curPage]=="vn" and (self.Emu.PC in self.memChanged) and not self.Emu.HALT: #(n < self.Emu.datamem[curPage] or
                    ...
                    self.memLabel[index].config(fg=cf.memColour,bg=cf.membgColour, text=self.Emu.hx(memval))#
                    
                if curPage==0 and n == self.Emu.datamem[0]: # First page of data (RAM) memory only, show IO ports
                    ...
                    if n == self.Emu.datamem[0]: #adr == indadr: # Only page 0 has IO
                        ...
                        for port in self.IOPorts:
                            ...
                            if memaddr in port.getIPadr():
                                self.memLabel[index].config(text=("%02X" % port.portIPvals[memaddr]))
                        ...
                        if memaddr in ipadrs or memaddr in opadrs:
                            ...
                            self.memLabel[index].config(bg=bgcolour, fg=fgcolour)#, text=ptext)#self.Emu.hx(memval))
                index += 1
        self.dispPC()
        return
    ...
    def dispPC(self):
        ...
        if not startIndex: # memory address not found in mcode_list
            ...
            while not startIndex:
                startIndex = self.mcode_list.search(self.Emu.hx((self.Emu.PC - n))+":","1.0", tk.END, regexp=True)
                #print("startIndex=", startIndex, type(startIndex))debug
                n += 1
                if n > nolines: break # Abandon search after all lines previous searched!
            
        else:# startIndex valid line
            # So ignore all lines if they contain a "set" directive
            linestr = self.asstxt.get("%s linestart" % startIndex, "%s lineend" % startIndex)
            if ("#" in linestr): linestr = linestr.split("#")[0]
            while ("set " in  linestr.lower()):#.find("set ")>-1):
                startIndex = self.mcode_list.search(self.pcLabVal.cget('text')+":",
                              "%s lineend" % startIndex, tk.END, regexp=True)
        ...
        if startIndex and startIndex != "1.0": # Then highlight current line if found in Mcode panel
            ...
            if "halt" in self.asstxt.get("%s linestart" % startIndex, "%s lineend+1c" % startIndex):
                 
                self.asstxt.tag_config("pc", underline=1, foreground=cf.PCcolour, background="white")
                self.mcode_list.tag_config("pc", underline=1, foreground=cf.PCcolour, background="white")
            else:
    ...
    def dispSP (self):
        ...
        if self.Emu.SP[stackpage] != 0: #self.runFrom.get():
            self.memLabel[self.Emu.SP[stackpage]+self.Emu.datamem[stackpage]*256].config(bg=cf.SPcolour, fg="black")
    ...
    def runProg(self, event=None):
        ...
        if self.running: # Then stop
            ...
            if runAction == 3: # Step
                ...
                while self.running and (not self.Emu.HALT):
                    #print(self.running, self.Emu.HALT)#debug
                    self.step()
                    if self.Emu.PC in self.Emu.BP: break # Break point detected
                    time.sleep(0.3)
            else:
                #Fast run 
                self.update()
                ...
                while self.running and not self.Emu.HALT:
                    ...
                    if runAction == 1 or stepcount == 0: # Fast speed!
                        self.step()
                    if runAction == 0 and stepcount < random.randint(21, 500): # Full speeed
                        stepcount += 1
        ...
        if self.Emu.HALT: 
            ...
            self.runStopButton.config(text="Start ", fg="black", activeforeground="black", state="disabled" )
        else:
            self.runStopButton.config(text="Start ", fg="black", activeforeground="black", state="normal")
        self.updateOPs()
        self.updateDisp()
        self.running=False
                            ...
                            for watchword in cf.watchtrigs:
                                if watchword == word:
                                    ...
                                    if fmtStr:
                                        ...
                                        fmtStr=0
                                    self.watches.append([int(float(startIndex))-1, None, None, fmtStr, 0, 0]) #[lineno, label, adr, disp, items, page]
        ...
        if self.amesRunning:
            ...
            if self.asstxt.edit_modified():
                if messagebox.askyesno("Quit","Do you want to save the file..."):
                        self.file_save()
        self.after(100, self.master.destroy) # Wait for everthing to copmplete to avoid erros
        return
    ...
    def file_save(self, event=None):
        ...
        if not self.file_path or self.file_path == "./" or ("Untitled" in self.file_path):
    ...
    def compileText(self, event=None):
        ...
        asectpos = self.asstxt.search('asect', "1.0", tk.END) # get first asect position
        if asectpos:
            asectadr = self.asstxt.get(asectpos+" +5c", asectpos+" lineend").split()[0]
        ...
        if errorMsg==None and obj_code != None and codelist != None:
                ...
                for watch in self.watches:
                    ...
                    if index == watch[0]: # Is a watch line address?
                        ...
                        if ":" in line:
                            ...
                            linno = str(index+1)+".0"
                            watch[5] = page
                            if watch[3] == None: # If no formatting overide metacommand, then set defaults
                        ...
                        if "ABS" in line:
                            ...
                            for val in memlist: # Check here for overlapping sects
                                if memadr > 255:
                                    ...
                                    if overlapStart: # Check for overalapping sections/asects
            ...
            if not errorMsg:
                ...
                if self.file_path:
                    ...
                    fname = "linkout"
                errorMsg, listing, IMG = cocol.link(objectfiles=linkfiles, ideobjtext=obj_code, filename=fname, termp=False, fileout=False)
        ...
        if errorMsg:
            ...
            #print(errorMsg)#DEBUG
            while "" in errorMsg: errorMsg.remove("")
            if len(errorMsg)>5:
                errorMsg.insert(4, " \n")
            msg = ""
