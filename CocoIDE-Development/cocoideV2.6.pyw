...
# V1.8      Harvard/VN Arch
# V1.91     Paged memory + interrupts.
# V1.92     MAC Notebook bug fixed.
# V1.93     Keyboard IO and HW Interrupts
# V1.94/5   Fixed mem display colours, added "wait" instruction to emeulator, and fixed bug in Interrupt rti
#           Timer. bug fixes (MAC workaround -> IO Buttons, cocas set directive,
# V1.97     Disabled paged memeory - stable production version
# V1.98     Fixed tooltip bug, revised "halt" behaviour.
# V1.99     Bug fixes:
# V1.901    V Flag regresssion fixed
# V2        More Robust Network checking for use with VPN!
# V2.1      Compile bug (outfile name not soecified when AMES running)
# V2.2      Suppress outfile.img when compiling/linking from CocoIDE
# V2.3      Bug fixes for online use with VPN
# V2.4      Bug fix for MAC/OS??
# To do:    Link files IDE directive (in compileTest())
#           Re-write compileText for multi page and compiler directives.
#           Memory Manager - in progress

title = "CocoIDE V2.4"  # Should be updated to reflect version
...
class CreateToolTip(object):
    ...
    def schedule(self):
        self.unschedule()
        self.afterid = self.widget.after(self.waittime, self.showtip)
    ...
    def showtip(self, event=None):
        ...
        x += self.widget.winfo_rootx() - 4 * self.widget.winfo_width() + 10
...
class VerticalScrolledFrame(tk.Frame):
    ...
    * This frame only allows vertical scrolling
    ...
    def __init__(self, parent, *args, **kw):
        ...
        self.interior_id = canvas.create_window(
...
class CocoIDE(tk.Frame):
    def __init__(self, Emulator=None, master=None, filename=None, name="cocoide"):
        ...
        self.filemenu.add_command(
            label="New", command=self.file_new, accelerator=comkey + "n"
        )
        self.filemenu.add_command(
            label="Open", command=self.file_open, accelerator=comkey + "o"
        )
        self.filemenu.add_command(
            label="Save", command=self.file_save, accelerator=comkey + "s"
        )
        self.filemenu.add_command(
            label="SaveAs", command=self.file_save_as, accelerator=comkey + "S"
        )
        self.filemenu.add_separator()
        self.filemenu.add_command(
            label="Quit", command=self.close_window, accelerator=comkey + "q"
        )  # self.quit)
        self.menubar.add_cascade(
            label="File", menu=self.filemenu, accelerator=comkey + "f"
        )

        # Edit menu
        self.editmenu = tk.Menu(self.menubar, tearoff=0)
        self.editmenu.add_command(
            label="Undo", command=self.undo, accelerator=comkey + "z"
        )
        self.editmenu.add_command(
            label="Redo", command=self.redo, accelerator=comkey + "y"
        )
        self.editmenu.add_command(
            label="Cut", command=self.cut, accelerator=comkey + "x"
        )
        self.editmenu.add_command(
            label="Copy", command=self.copy, accelerator=comkey + "c"
        )
        self.editmenu.add_command(
            label="Paste", command=self.paste, accelerator=comkey + "v"
        )
        self.editmenu.add_separator()
        self.txtmenu = tk.Menu(self.editmenu, tearoff=0)
        self.txtmenu.add_radiobutton(
            label="V Large", command=lambda: self.changeTextSize(16)
        )
        self.txtmenu.add_radiobutton(
            label="Large", command=lambda: self.changeTextSize(12)
        )
        self.txtmenu.add_radiobutton(
            label="Default", command=lambda: self.changeTextSize(10)
        )
        self.txtmenu.add_radiobutton(
            label="Small", command=lambda: self.changeTextSize(8)
        )
        ...
        self.emumenu.add_command(label="Start", command=self.runProg)
        ...
        tk.Button(self.editButtons, text="Search", command=self.searchText).grid(
        ...
        tk.Button(self.editButtons, text="Line", command=self.gotoLine).grid(
        ...
        if platform == "darwin":
            self.saveImageButton = tk.Button(
                buttonBar, text="Save Image ", height=3, command=self.saveImage
            )
        else:
            self.saveImageButton = tk.Button(
                buttonBar, text="Save Mem\nImage ", height=3, command=self.saveImage
            )
        ...
        self.runStopButton = tk.Button(
            buttonBar, text="Start ", height=3, padx=6, width=4, command=self.runProg
        )
        self.runStopButton.pack(side=tk.RIGHT)

        self.runFromFrame = tk.Frame(buttonBar)
        self.runFromFrame.pack(side=tk.RIGHT)
        self.runFromLabel = tk.Label(self.runFromFrame, text="Start From")
        ...
        self.runEPSelect = ttk.Combobox(
            self.runFromFrame,
            textvariable=self.runFrom,
            state="readonly",
            width=6,
            foreground="black",
            background="white",
        )
        self.runEPSelect.bind(
            "<<ComboboxSelected>>", self.initPC
        ...
        # Status/Run time warning/Error pane
        ...
        mainPanel.rowconfigure(
            0, weight=1
        )  # Allow text and mcode windows to scale vertically
        mainPanel.columnconfigure(
            1, weight=1
        )  # Allow text and watch windows to scale horizontally
        ...
        self.asstxt = tk.Text(
            mainPanel,
            wrap=tk.NONE,
            font=self.defaulttxtfont,
            undo=True,
            yscrollcommand=self.yscroll1,
            autoseparators=True,
            maxundo=-1,
            width=40,
        )  # , height=10,) height=editorHeight,
        ...
        txtHscroll = ttk.Scrollbar(
            mainPanel, orient=tk.HORIZONTAL, command=self.asstxt.xview
        )
        ...
        self.asstxt.config(
            font=self.boldfont, tabs=tab_width, tabstyle="wordprocessor"
        )  # "1.0c 2.0c 3.0c")#tab_width,))

        ## Create the machine code memory list display panel
        mcode_frame = ttk.Frame(mainPanel)  # , width=60)
        mcode_frame.grid(
            row=0, column=3, sticky="nsew"
        )  # pack(fill=tk.Y, expand=1, side=tk.LEFT)
        self.mcode_list = tk.Text(
            mcode_frame,
        ...
        mcodeHscroll = ttk.Scrollbar(
            mainPanel, orient=tk.HORIZONTAL, command=self.mcode_list.xview
        )
        ...
        watchPanel = tk.Frame(
            mainPanel, name="watchpanel"
        )  # , bg="blue")#,  height=80)#,  background="red") #width=570,
        watchPanel.grid(
        ...
        watchHeader = tk.Label(
            watchPanel, text="Pg:  Adr:  Label:               Content "
        )
        watchHeader.grid(
            row=1, column=0, columnspan=2, sticky="w"
        )  # pack(side=tk.TOP, fill=tk.X, anchor="w")

        self.watchList = tk.Text(
            watchPanel, height=9, bg="white", font=self.boldfont, wrap=tk.NONE
        )  # ,width=editorWidth-2, text='data: OE: "Hello there"')
        self.watchList.bind("<Key>", lambda e: "break")  # Disable editing
        self.watchList.grid(row=2, column=0, columnspan=2, sticky="nsew")
        watchScrollY = tk.Scrollbar(
            watchPanel, orient=tk.VERTICAL, command=self.watchList.yview
        )
        self.watchList.config(yscrollcommand=watchScrollY.set)
        watchScrollY.grid(row=2, column=1, sticky="nse")
        watchScrollX = tk.Scrollbar(
            watchPanel, orient=tk.HORIZONTAL, command=self.watchList.xview
        )
        ...
        self.regPanel = tk.Frame(
            mainPanel, name="regpanel", height=80, padx=0, relief="sunken", bd=2
        ...
        # Program counter
        self.pcLab = tk.Label(
            self.regPanel, text="^ PC  ", width=6, font=self.boldfont, padx=3
        )
        self.pcLab.grid(row=1, column=0, sticky="w")  # , columnspan=2)
        self.pcLabVal = tk.Label(
            self.regPanel,
            text="00",
            width=6,
            bg=cf.PCcolour,
        ...
        self.CVZN_Lab = tk.Label(
            self.regPanel, text="PS: I Page CVZN", width=15, font=self.boldfont
        )
        self.CVZN_Lab.grid(
            row=1, column=1, columnspan=2
        )  # ,sticky="e")#, columnspan=2)
        self.CVZN_Val = tk.Label(
            self.regPanel,
            text="0 000 0000",
            bg="white",
            width=15,
        ...
        self.pageRadButtons = []
        for n in range(8):
            self.pageRadButtons.append(
                tk.Radiobutton(
                    self.memPageFrame,
                    text=str(n),
                    variable=self.memPageVar,
                    value=n,
                    indicatoron=0,
                    width=3,
                    padx=4,
                    command=lambda: (
                        self.dispAllMemory(page=self.memPageVar.get()),
                        self.dispPC(),
                    ),
                )
            )
        ...
        self.IOLabel = tk.Label(
            self.IOHeader, font=self.boldfont, text="               I/O Ports"
        )
        self.IOLabel.pack(side=tk.LEFT)
        # IO ADD/DEL IO port Buttons
        self.portVar = tk.StringVar()
        self.portVar.set(self.ioPortnames[0])
        self.addPortSelect = ttk.Combobox(
            self.IOHeader, textvariable=self.portVar, state="readonly", width=15
        )
        ...
        self.portVarLabel = tk.Button(
            self.IOHeader, text="+", command=self.addport, width=1
        )
        ...
        self.IOscroll = ttk.Scrollbar(
            self.IOPanel, orient=tk.VERTICAL
        )  # , command=self.IOcanvas.yview)
        self.IOscroll.pack(
            side=tk.RIGHT, fill=tk.Y, expand=0
        )  # row=0, column=2, sticky="ns")
        self.IOcanvas = tk.Canvas(
            self.IOPanel, yscrollcommand=self.IOscroll.set
        )  # bg="yellow")
        ...
        self.IOFrame_win = self.IOcanvas.create_window(
            0, 0, window=self.IOFrame, anchor=tk.NW
        )
        ...
        def _configure_IOcanvas(event=None):
            ...
            if self.IOFrame.winfo_reqwidth() != self.IOcanvas.winfo_width():
                # update the inner frame's width to fill the canvas
                self.IOcanvas.itemconfigure(
                    self.IOFrame_win, width=self.IOcanvas.winfo_width()
                )
        ...
        for port in self.IOPorts:
            for adr in port.portIPvals.keys():
                ...
                if adr == self.Emu.ipAdr:
                    ...
                    port.getIPval()  # trigger port update
        self.updateDisp()
        return
    ...
    def updateOPs(self):
        ...
        if self.Emu.memChanged[0]:
            adr = self.Emu.memChanged[0][-1]  # last change only, always page 0
            for port in self.IOPorts:
                # print(adr,"**",list(port.getOPadr()))
                if adr in port.getOPadr():
                    errormsg = None
                    # print("*", adr, list(port.getOPadr()), memval)
                    errormsg = port.setOPval(
                        adr, self.Emu.memory[0][self.Emu.datamem[0]][adr]
                    )
                    if errormsg:
                        # rint(errormsg)
                        txtaddr = self.mcode_list.search(
                            self.Emu.hx(self.Emu.PC - 1) + ":", "1.0", tk.END
                        )
                        if txtaddr:
                            # print("£",txtaddr)
                            self.mcode_list.tag_add(
                                "smod",
                                "%s linestart" % txtaddr,
                                "%s lineend+1c" % txtaddr,
                            )  # add tag to k
                            self.mcode_list.tag_config(
                                "smod", font=self.boldfont, foreground="red"
                            )
                        self.statusMsg.config(text=errormsg)
                    try:
                        del self.Emu.memChanged[0][-1]
                    except IndexError:  # When user hits compile while running!
                        pass
                        # print(self.Emu.memChanged[0])
    ...
    def amesStart(self, event=None):
        ...
        if errormsg == None:  # No network errors etc.
            ...
            if (
                success
            ):  # and self.timeleft>0: #if errormsg == None: # commnent out for testing w out ames.
            ...
            # or self.timeleft <= 0:
            if self.timeleft <= 0:
                self.statusMsg.config(text="AMES: TEST TIMED OUT")
                #
            # else:
        if errormsg:
            self.statusMsg.config(text=errormsg)
            ...
            self.amesButton.config(
                text="Submit", state=tk.NORMAL, command=self.amesSubmit
            )
    ...
    def amesClockUpdate(self, timeleft=None):
        ...
        if self.amesRunning and (
            self.timeleft > 0 or self.timeleft < -11
        ):  # Still time left?
            ...
            self.pinLabel.config(
                text="AMES <" + str(int(self.timeleft // 60) + 1) + " MIN LEFT",
                fg="white",
                bg="black",
            )
    ...
    def amesSubmit(self, ext="asm", auto=False):
        ...
        if content and self.amesRunning:
            ...
            if not errormsg:
                if self.timeleft > 0:
                    if not auto:
                        self.statusMsg.config(text="SUBMITTED OK")
    ...
    def amesStop(self):
        ...
        if self.amesSubmit() == True:  # if file submitted ok
            if self.timeleft >= 0:
            ...
            try:
                ...
                self.file_new()
        else:  # Problem submitting
            self.statusMsg.config(
                text="Problem submitting file:\nPlease Save file as yourSRN.asm\n and Email to Tutor"
            )
            self.file_save_as()
            self.file_new()
        # Then clear up and restore normal editing etc.
    ...
    def bindKeys(self):
        ...
        self.asstxt.bind("<Control-C>", self.copy)
        self.asstxt.bind("<Control-x>", self.cut)
        self.asstxt.bind("<Control-X>", self.cut)
    ...
    def keydisable(self, event=None):
        ...
        if self.amesRunning and event.keysym not in [
            "Left",
            "Right",
            "Up",
            "Down",
            ...
            "Prior",
            "Next",
        ]:  # , "Control_R"]:# Ames backdoor
            # Do not allow editing of #! lines, or inserting ! after #
            if event.keysym == "exclam" and self.asstxt.search(
                "#", "insert-1c", "insert"
            ):
                return "break"
    ...
    def initMemDisplay(self, page=None):
        ...
        for n in range(len(self.mem_frame)):
            ...
            for col in range(16):
                tk.Label(
                    self.mem_frame[n],
                    text=self.Emu.hx(col)[1] + " ",
                    font=self.boldfont,
                ).grid(row=0, column=gridpos, padx=2, pady=2)
                tk.Label(
                    self.mem_frame[n],
                    text=self.Emu.hx(col)[1] + " ",
                    font=self.boldfont,
                ).grid(row=gridpos, column=0, padx=2, pady=2)
                gridpos += 1
    ...
    def dispAllMemory(self, page=None):  # ??
        ...
        if self.Emu.arch[curPage] == "vn":
            numbanks = 1
            txt = "Page " + str(curPage) + " Memory"
            self.emuNb.add(self.mem_frame[0])  # , text=txt)
            self.emuNb.tab(0, text=txt)
            self.emuNb.hide(1)
            self.emumenu.entryconfig(8, label="Arch. = Harvard   ")
        else:
            numbanks = 2
            txt1 = "Page " + str(curPage) + " ROM"
            txt2 = "Page " + str(curPage) + " RAM"
            self.emuNb.add(self.mem_frame[0])  #
            self.emuNb.tab(0, text=txt1)
            self.emuNb.add(self.mem_frame[1])
            self.emuNb.tab(1, text=txt2)  # , state="normal")#, underline=0, padding=2)
        ...
        for n in range(numbanks):  # All banks in in current CDM8 memory page
            ...
            for memval in self.Emu.memory[curPage][
                n
            ]:  # self.Emu.curPage][n]: # For each bank
                self.memLabel[index].config(
                    text=self.Emu.hx(memval), fg=cf.memColour, bg=cf.membgColour
                )  # memColour
                ...
                if self.memChanged:
                    ...
                    if (
                        index == 0
                        or self.Emu.PC == self.runDict[self.runFrom.get()] == 0
                    ):  # Except at start of run
                        self.memLabel[index].config(fg=cf.memColour)
                    elif (
                        n == self.Emu.datamem[curPage]
                    ):  # If n = 0, then VN Arch, else 1 = HV Arch
                        for addr in self.memChanged:
                            if (
                                index % 256
                            ) == addr:  # Highlight if memory cell has changed
                                # print(index, int(addr))# debug
                                self.memLabel[index].config(fg=cf.chMemColour)

                    # Runtime Self modifying code warning
                    if (
                        self.arch[curPage] == "vn"
                        and (self.Emu.PC in self.memChanged)
                        and not self.Emu.HALT
                    ):  # (n < self.Emu.datamem[curPage] or
                        self.statusMsg.config(
                            text="Runtime Warning:\nExecuting Self\nModified Code?"
                        )
                        txtaddr = self.mcode_list.search(
                            self.Emu.hx(self.Emu.PC) + ":", "1.0", tk.END
                        )
                        if txtaddr:
                            self.mcode_list.tag_add(
                                "smod",
                                "%s linestart+3c" % txtaddr,
                                "%s lineend+1c" % txtaddr,
                            )  # add tag to k
                            self.mcode_list.tag_config(
                                "smod", font=self.boldfont, foreground="red"
                    ...
                    self.memLabel[index].config(
                        fg=cf.memColour, bg=cf.membgColour, text=self.Emu.hx(memval)
                    )  #

                if (
                    curPage == 0 and n == self.Emu.datamem[0]
                ):  # First page of data (RAM) memory only, show IO ports
                    # Colour IO port adresses
                    memaddr = index - (self.Emu.datamem[0] * 256)
                    if n == self.Emu.datamem[0]:  # adr == indadr: # Only page 0 has IO
                        ipadrs = []
                        opadrs = []
                        for port in self.IOPorts:
                            # update port displays
                            if memaddr in port.getIPadr():
                                self.memLabel[index].config(
                                    text=("%02X" % port.portIPvals[memaddr])
                                )
                            # elif memaddr in port.getOPadr():
                            #    self.memLabel[index].config(text="00")
                            ipadrs += port.getIPadr()
                            opadrs += port.getOPadr()
                        if memaddr in ipadrs or memaddr in opadrs:
                            # set port colours
                            if memaddr in ipadrs and memaddr in opadrs:
                                fgcolour = cf.membgColour
                                bgcolour = cf.ioportColour
                            elif memaddr in opadrs:
                                fgcolour = cf.oportColour
                                bgcolour = cf.oportColour
                            elif memaddr in ipadrs:
                                fgcolour = cf.membgColour  # = "white"
                                bgcolour = cf.iportColour
                            self.memLabel[index].config(
                                bg=bgcolour, fg=fgcolour
                            )  # , text=ptext)#self.Emu.hx(memval))
                index += 1
        self.dispPC()
        return
    ...
    def dispPC(self):
        ...
        # if self.mcode_list.get("1.0", tk.END) != "\n": #mcode_list is empty
        # self.pcLabVal.cget() returns hex PC address as string
        startIndex = self.mcode_list.search(
            self.pcLabVal.cget("text") + ":", "1.0", tk.END, regexp=True
        )
        # print("Mcode Line no=",startIndex, "*")# debug
        nolines = int(self.mcode_list.index("end-1c").split(".")[0])
        if not startIndex:  # memory address not found in mcode_list
            # Then search backward from current PC to pick up last valid memeory address in mcode
            # normally the "halt" instruction
            n = 1
            while not startIndex:
                startIndex = self.mcode_list.search(
                    self.Emu.hx((self.Emu.PC - n)) + ":", "1.0", tk.END, regexp=True
                )
                # print("startIndex=", startIndex, type(startIndex))debug
                n += 1
                if n > nolines:
                    break  # Abandon search after all lines previous searched!

        else:  # startIndex valid line
            # So ignore all lines if they contain a "set" directive
            linestr = self.asstxt.get(
                "%s linestart" % startIndex, "%s lineend" % startIndex
            )
            if "#" in linestr:
                linestr = linestr.split("#")[0]
            while "set " in linestr.lower():  # .find("set ")>-1):
                startIndex = self.mcode_list.search(
                    self.pcLabVal.cget("text") + ":",
                    "%s lineend" % startIndex,
                    tk.END,
                    regexp=True,
                )
                if startIndex:  # ignore all lines with "set"
                    linestr = self.asstxt.get(
                        "%s linestart" % startIndex, "%s lineend" % startIndex
                    )
                    if "#" in linestr:
                        linestr = linestr.split("#")[0]
                else:  # Address not found in Mcode panel
                    break

        if (
            startIndex and startIndex != "1.0"
        ):  # Then highlight current line if found in Mcode panel
            # Highlight in mcode panel
            self.mcode_list.tag_delete("pc")
            self.mcode_list.tag_add(
                "pc", "%s linestart+3c" % startIndex, "%s lineend+1c" % startIndex
            )  # add tag to k

            # and in asstxt panel
            self.asstxt.tag_delete("pc")
            self.asstxt.tag_add(
                "pc", "%s linestart" % startIndex, "%s lineend+1c" % startIndex
            )  # add tag to k

            if "halt" in self.asstxt.get(
                "%s linestart" % startIndex, "%s lineend+1c" % startIndex
            ):

                self.asstxt.tag_config(
                    "pc", underline=1, foreground=cf.PCcolour, background="white"
                )
                self.mcode_list.tag_config(
                    "pc", underline=1, foreground=cf.PCcolour, background="white"
                )
    ...
    def dispRegs(self):
        for index in range(4):
            self.regHexs[index].config(
                text="0x" + self.Emu.hx(self.Emu.regs[index])
            )  # hex row
            self.regStrs[index].config(
                text=self.Emu.convert(2, self.Emu.regs[index])
            )  # Char row

            self.regDecs[index].config(
                text="%+04d" % int(self.Emu.convert(1, self.Emu.regs[index]))
                + " %03d" % self.Emu.regs[index]
            )  # dec row
            # print("*"+convert(1,Regs[k]))# debug
            self.regBins[index].config(
                text=format(self.Emu.regs[index], "08b")
            )  # Bin row
        return
    ...
    def dispSP(self):
        ...
        self.memLabel[self.prevSP + self.Emu.datamem[stackpage] * 256].config(
            bg="white"
        )
        if self.Emu.SP[stackpage] != 0:  # self.runFrom.get():
            self.memLabel[
                self.Emu.SP[stackpage] + self.Emu.datamem[stackpage] * 256
            ].config(bg=cf.SPcolour, fg="black")
    ...
    def step(self, dispUD=True):
        # interrupt = cdm8_io.interrupt
        vectors = cdm8_io.interruptVectors  # fetch vectors, if any
        # print("£", vectors)
        self.Emu.step(vectors)
        # Update the interrupt vector queue to the IO devices
        cdm8_io.interruptVectors = self.Emu.intvectors
        self.updateOPs()
        if dispUD:
            self.updateDisp()
    ...
    def runProg(self, event=None):
        ...
        if self.running:  # Then stop
            self.running = False
            # self.Emu.HALT=True
        elif self.Emu.HALT != True:  # processor not halted
            self.statusMsg.config(text="")
            if runAction == 3:  # Step
                # Step
                self.step()
                ...
                while self.running and not self.Emu.HALT:
                    ...
                    if runAction == 1 or stepcount == 0:  # Fast speed!
                        self.step()
                    if runAction == 0 and stepcount < random.randint(
                        21, 500
                    ):  # Full speeed
                        stepcount += 1
                        self.step(dispUD=False)
                    else:  # stepcount >= 91
                        stepcount = 0
                    if self.Emu.PC in self.Emu.BP:
                        self.updateDisp()
                        # print("HALT=", self.Emu.HALT)# debug
                        break  # Break point detected

        if self.Emu.HALT:
            self.statusMsg.config(text="Processor Halted: Reset to Run Program")
            self.runStopButton.config(
                text="Start ", fg="black", activeforeground="black", state="disabled"
            )
        else:
            self.runStopButton.config(
                text="Start ", fg="black", activeforeground="black", state="normal"
            )
        self.updateOPs()
        self.updateDisp()
        self.running = False
        self.runStopButton.config(text="Start ", fg="black", activeforeground="black")
    ...
    def highlighter(self, event=None):
        ...
        if event != None:
            ...
            if event.keysym == "Return":
                ...
                while startIndex:
                    startIndex = self.asstxt.search(
                        "\t", startIndex, "%s+1c" % startIndex
                    )
        ...
        self.asstxt.tag_config(
            "all", foreground="black", font=self.defaulttxtfont
        )  # font=self.boldfont)
        ...
        for fgcolour in cf.highlights:
            ...
            for word in cf.highlights[fgcolour]:  # iterate over directive words list
                ...
                while startIndex:
                    startIndex = self.asstxt.search(
                        "\\y" + word + "\\y", startIndex, tk.END, regexp=True
                    )
                    if startIndex:
                        endIndex = self.asstxt.index(
                            "%s+%dc" % (startIndex, (len(word)))
                        )  # find end of word
                        self.asstxt.tag_add(word, startIndex, endIndex)  # add tag to k
                        self.asstxt.tag_config(
                            word, foreground=fgcolour, font=self.boldfont
                        )  # and color it with v
                        startIndex = endIndex  # reset startIndex to continue searching

                        # Update Watches list
                        # if a line contains a dc or ds (and not commented!)
                        if not self.asstxt.search(
                            cf.commentprefix, "%s linestart" % startIndex, startIndex
                        ):
                            for watchword in cf.watchtrigs:
                                if watchword == word:
                                    fmtStr = self.asstxt.search(
                                        cf.commentprefix + "$",
                                        "%s linestart" % startIndex,
                                        "%s lineend" % startIndex,
                                    )
                                    if fmtStr:
                                        fmtStr = self.asstxt.get(
                                            "%s + 2c" % fmtStr, "%s + 5c" % fmtStr
                                        )
                                        # print("*"+fmtStr)# debug
                                    else:
                                        fmtStr = 0
                                    ...
                                    )  # [lineno, label, adr, disp, items, page]
                    ...
                    if not self.asstxt.search(cf.commentprefix, lineStart, startIndex):
                        ...
                        if tagname != "":  # Ignore if not a valid label
                            self.labelList.append(
                                tagname
                            )  # Here store lineno as well for runfrom??
                            self.asstxt.tag_add(
                                tagname, lineStart, startIndex
                            )  # add tag to k
                            self.asstxt.tag_config(
                                tagname, foreground=fgcolour, font=self.asstxt["font"]
                            )
                        ...
                        for index in range(len(self.watches)):
                            if lineno - 1 == self.watches[index][0]:
                                self.watches[index][
                                    1
                                ] = tagname  # Store label line num for adding watches
        ...
        for word in self.labelList:  # iterate over directive words list
            ...
            while startIndex:
                ...
                if startIndex:
                    # print(startIndex)# debug
                    lineStart = (
                        startIndex  # self.asstxt.index('%s linestart ' % startIndex)
                    )
                    endIndex = self.asstxt.index(
                        "%s+%dc" % (startIndex, (len(word)))
                    )  # find end of word
                    ...
                    # Check line not commented
                    if not self.asstxt.search(
                        cf.commentprefix, lineStart + " linestart", lineStart
                    ):
                        self.asstxt.tag_add(word, startIndex, endIndex)  # add tag to k
                        self.asstxt.tag_config(
                            word, foreground=fgcolour, font=self.boldfont
    ...
    def toggleBP(self, event=None):
        try:
            bpAdrStr = str(
                self.mcode_list.get("insert linestart", "insert linestart +2c")
            )
            ...
            if bpAdr in self.Emu.BP:
                ...
                for n in range(len(self.bpTagNames)):
                    if bpAdrStr == self.bpTagNames[n]:
                        del self.bpTagNames[n]
                ...
                self.mcode_list.tag_add(
                    bpAdrStr, "insert linestart", "insert lineend+1c"
                )
                ...
                self.asstxt.tag_add(
                    bpAdrStr, lineNum + " linestart", lineNum + " lineend+1c"
                )
    ...
    def updateLineNos(self, event=None):
        ...
        if (
            self.asstxt.index("end").split(".")[0]
            != self.lntext.index("end").split(".")[0]
        ):
            self.lntext.config(state="normal")
            self.lntext.delete(1.0, tk.END)
            for n in range(1, int(self.asstxt.index("end").split(".")[0])):
                self.lntext.insert(tk.END, str(n) + "\n")
            self.lntext.delete("end -2c", tk.END)  # adjust lines as tk adds a \n
            first, last = self.asstxt.yview()
            self.lntext.yview_moveto(first)
            self.lntext.config(state="disabled")
        return

    # Editor right click Popup menu for asstxt widget
    ...
    # Mcode right click listing Popup memu for mcode_list widget
    ...
    # Catch window close event aand clear up first!
        ...
        if self.amesRunning:
            ...
            if self.asstxt.edit_modified():
                if messagebox.askyesno("Quit", "Do you want to save the file..."):
                    self.file_save()
        self.after(
            100, self.master.destroy
        )  # Wait for everthing to copmplete to avoid erros
        return
    ...
    def file_save(self, event=None):
        ...
        # print("file_path", self.file_path)# debug
        if (
            not self.file_path
            or self.file_path == "./"
            or ("Untitled" in self.file_path)
        ):
    ...
    def file_save_as(self, event=None, filepath=None, ext=".asm", text=None):
        if ext == ".asm":
            filetype = "CDM8 Assembly"
        if ext == ".obj":
            filetype = "CDM8 Object File"
        if filepath == None:
            if self.file_path:
                ...
                filepath = filedialog.asksaveasfilename(
                    ...
                    initialfile=self.file_path.split("/")[-1],
                )
            else:
                filepath = filedialog.asksaveasfilename(
                    filetypes=((filetype, "*" + ext), ("All files", "*.*")),
    ...
    def cut(self, event=None):
        try:
            ...
            if self.amesRunning and ("#!" in self.cliptext):  # DO not delete #! lines
                ...
                self.amesRunning
                and self.asstxt.search("#!", "insert linestart", "insert lineend")
            ):
        ...
        try:
            if self.amesRunning:  # Do not use system clipboard
                ...
                if self.asstxt.search("#!", "insert linestart", "insert lineend"):
                    ...
                    self.cliptext = ""
                if (
                    self.asstxt.search("#", "insert-1c", "insert")
                    and self.cliptext == "!"
                ):
                    self.cliptext = ""
                if (
                    self.asstxt.search("!", "insert", "insert+1c")
                    and self.cliptext == "#"
                ):
                    self.cliptext = ""
            else:
    ...
    def searchText(self, event=None):
        try:
            ...
            if searchStr:
                ...
                endIndex = self.asstxt.index(
                    "%s+%dc" % (self.startIndex, (len(searchStr)))
                )  # find end of word
    ...
    def toggleMemPageDisp(self, event=None):
        if self.pageDisp:
            ...
            self.Emu.curPage = 0
            self.memPageVar.set(0)
            self.initMemDisplay(page=0)
            self.dispAllMemory(page=0)
        else:
            ...
            with open(filepath, "wb") as f:
                ...
                for memVal in self.Emu.memory[0][0]:
                    try:  # Python3
                        ...
                        f.write(
                            bytes(self.Emu.convert(0, memVal) + "\n")
                        )  # , 'UTF-8'))
                return "saved"
        # except IOError:
        #    return "Cancelled"
    ...
    def compileText(self, event=None):
        ...
        ## Global settings
        # Set IO devices #$io=<device>, address

        # Include/Link search folder path(s)  #$path=<path1>;<path2>;...;<pathn>
        try:
            fpath = os.path.dirname(self.file_path)  # current directory
        except:
            fpath = "."
        # print("path=", fpath)

        ## Page specific settings
        # Set Page # #$page=<0 | 1 | 2| 3| 4| 5| 6| 7>. Default = 0

        # Set architecture, page specific. # Harvard $arch=hv, Von Neuman #$arch=vn
        ...
        # Set Linked file(s), page specific #$link=<filename1>;<filename2>;....;<filenamen>
        objfilename = []
        linkpos = self.asstxt.search("#$link=", "1.0", tk.END)
        linkfiles = []
        if linkpos:
            linktxt = self.asstxt.get(linkpos + "+7c", linkpos + " lineend")
            try:
                linktxt = linktxt.split("#")[0]
                linkfiles = linktxt.split[0](",")
            except TypeError:
                pass
            index = 0
            for fname in linkfiles:
                # print(fname)#debug
                linkfiles[index] = fpath + "/" + fname.lstrip()
                index += 1
        else:
            linkfiles = []
        # print(linkfiles)#debug

        ## Call the Compiler (cocas.py)
        # Warn if program does not start at address 0
        asectpos = self.asstxt.search(
            "asect", "1.0", tk.END
        )  # get first asect position
        if asectpos:
            asectadr = self.asstxt.get(
                asectpos + " +5c", asectpos + " lineend"
            ).split()[0]
            # print(asectadr)#debug
        ...
        if errorMsg == None:
            ...
            try:
                obj_code, codelist, errorMsg = cocas.compile_asm(filebuff, self.cdm8ver)
            except Exception as e:
                errorMsg = e

        # print("33", errorMsg)#debug
        # If no errors, load CocoEmu, and Update the GUI display
        if errorMsg == None and obj_code != None and codelist != None:
            strippedCode = ""
            ...
            for line in strippedCode.splitlines():
                ...
                # Watch formatting directives: #!hex, #!dec,  #!str or #!bin
                for watch in self.watches:
                    # print(index, watch[0])#debug
                    if index == watch[0]:  # Is a watch line address?
                        # print(line)
                        if ":" in line:
                            ...
                            if type(watchLine[1]) == list:
                                watch[4] = (
                                    len(watchLine[1]) - 1
                                )  # split(" ") inserts an empty item in list???
                            else:
                            ...
                            if (
                                watch[3] == None
                            ):  # If no formatting overide metacommand, then set defaults
                                if self.asstxt.search('"', linno, "%s lineend" % linno):
                                    watch[3] = "str"
                                elif self.asstxt.search(
                                    "0b", linno, "%s lineend" % linno
                                ):
                                    watch[3] = "bin"
                                elif self.asstxt.search(
                                    "0x", linno, "%s lineend" % linno
                                ):
                                    watch[3] = "hex"
                                else:
                                    watch[3] = "dec"
            ...
            # ??? This needs rewriting and tidying up!!!
            memChunks = obj_code.splitlines()
            lineNum = 1
            memadr = 0  # "00"
            for line in memChunks:
                # print("*"+line)#[:4])#debug
                if ("ABS" in line) or ("DATA" in line):
                    if "ABS" in line:
                        mem = [line.lstrip("ABS").split(":")]
                    elif "DATA" in line:  # Not too sure about this???
                        mem = [str(memadr), line.lstrip("DATA")]  # .split(" ")
                        # print("£", memadr, mem)#debug
                        memlist = mem[1].lstrip(" ").split(" ")
                    for item in mem:
                        if "ABS" in line:
                            memadr = int("0x" + item[0].lstrip(" "), 16)
                            memlist = item[1].lstrip(" ").split(" ")
                            # print(memadr, "memlist",memlist)
                            for val in memlist:  # Check here for overlapping sects
                                if memadr > 255:
                                    ...
                                    self.Emu.memory[0][0][memadr] > 0
                                ):  # Already occupied by code?
                                    # print(memadr, lineNum, line)
                                    # Find line where asect overlaps?? Problem here??
                                    overlapStart = None
                                    overlapStart = self.mcode_list.search(
                                        "%02x:" % memadr, "1.0", stopindex="end"
                                    )
            ...
            if not errorMsg:
                # if any linked files, link them here, then (re)load the memory image from the linker.
                # Will resolve all ext and entry addresses Can also generate an error message.
                # print("Linking", self.file_path, linkfiles )
                if self.file_path:
                    fname = self.file_path[:-4]  # os.path.basename(self.file_path)[:-4]
                else:
                    fname = "linkout"
                errorMsg, listing, IMG = cocol.link(
                    objectfiles=linkfiles,
                    ideobjtext=obj_code,
                    filename=fname,
                    termp=False,
                    fileout=False,
                )
                # print("**",errorMsg, listing, IMG)#debug

            if not errorMsg:
                # Load linked image to emulator (even if not needed?
                index = 0
                for num in IMG:
                    self.Emu.memory[0][0][index] = num
                    index += 1

        # End of compile/link/load mem

        # If error/warning show message in mcode window
        if errorMsg:
            # print("*"+errorMsg)#DEBUG
            errorMsg = str(errorMsg).split(" ")
            # print(retError)
            self.mcode_list.delete(1.0, tk.END)
            ...
            for n in range(
                ...
                len(self.Emu.memory[self.Emu.curPage]),
            ):  # Clear data memory
    ...
    def highlightLine(self, lintxt=None, linend=None):
        ...
        if lintxt:
            self.asstxt.tag_add(
                "err", lintxt, linend
            )  # "%s linestart" % (lintxt), "%s lineend+1c" % (lintxt)) # add tag to k
            self.asstxt.tag_config("err", background=cf.errColour)
            self.update()
        ...
        for label in self.labelList:
            if cf.entrySpec in label[0]:
                ...
                if runAddr != "" and runAddr != "0.0":
                    self.runDict[label] = int(
                        self.mcode_list.get("%s-2c" % runAddr, runAddr), 16
                    )
    ...
    def updateWatchWin(self):
        ...
        for watch in self.watches:
            ...
            if watch[2]:
                ...
                if watch[1]:
                    watchLine = str(watch[5]) + "  " + watch[2] + "   " + watch[1]
                else:
                    watchLine = str(watch[5]) + "  " + watch[2]
...
#### Final error save files! Disabled currently!
def savefiles():
