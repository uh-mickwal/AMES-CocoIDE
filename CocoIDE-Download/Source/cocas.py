...
class CocAs(tk.Tk):
    def __init__(self, master=None, name="cocas", exitroot=False, sym=True):
        ...
        if __name__ == "__main__": self.mainWin.protocol("WM_DELETE_WINDOW",self.closeCocas) #Only if main module
        self.mainWin.focus()
        ...
        # buttons
        addButton = tk.Button(buttonBar, text="Select .asm File", command=self.addFile)#, height=2)
        ...
        linkButton = tk.Button(buttonBar, text="Assemble to .obj File", command=lambda: self.asmFile(sym=sym))#, height=2)
        linkButton.pack(side=tk.LEFT)
        #linkButton = tk.Button(buttonBar, text="Asse Logisim Image", command=self.linkFiles)#, height=2)
        #linkButton.pack(side=tk.LEFT)
        quitButton = tk.Button(buttonBar, text="Quit", command=self.closeCocas)#, height=2)
    ...
    def addFile(self, event=None):
        ...
        try:
            filepath = filedialog.askopenfilename(filetypes=(('CDM8 Assembly File', '*.asm'), ('All files', '*.*')))
        except:
            pass
        self.mainWin.lift()
        if filepath:
...
def lex(s):
    ...
    x=s[i]
    if x.isalpha() or x=="_" or x=="*": CAT="id"
...
def lexline(linum,s):
    ...
    while ind>=0:
        ...
        try:
            ...
            EP( "On line "+str(linum)+" \n"+str(s0[0:ptr+e.ind])+str(s0[ptr+e.ind:])+"\nERROR: "+e.msg )
    ...
    def parse_exp(lst,onlyabs=False):
        ...
        if opsynt[0]=="id":
            ...
            if SectName != -1 and lbl in labels[SectName]:
                ...
                if relcontext and lbl not in exts: gotrel=True
            elif lbl in abses:
                Value=abses[lbl]
                gotrel=False
            elif lbl=='*':
            ...
            if opsynt[2]=="id":
                lbl2=lst[2][1]
                if lbl2 in exts:
                    raise SE(lst[2][2],"External label "+lbl2+" can't be used as displacement")
    ...
    if cmd[0][0]!="id":
        ...
        if cat==spec:
            ...
            if opcode=="set":
                ...
                if passno==2: return (label,-10,[Value])
                alias=cmd[next][1]
                if alias in abses:
    ...
    for passno in [1,2]:
        ...
        while True:
            ...
            if size == -1:      # sects
                ...
                if passno==2:
                    output += [(linind,code[0],[],"")]      # dummy output to get addresses in listing
                continue
...
def genoc(output, objbuff=None):
    ...
    for pair in absegs:
        (a,d)=pair
        if objbuff==None:
            objfile.write("ABS  "+format(a,"02x")+": "+' '.join(map(shex,d))+'\n')
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l',dest='lst',action='store_const',const=True, help="Produce program listing")
